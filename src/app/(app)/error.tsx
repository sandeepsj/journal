  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="text-center space-y-4 max-w-sm animate-page-enter">
        ...
        <h2 className="font-serif text-2xl text-[var(--color-text-primary)]">Something went wrong</h2>
        <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">
          {error.message || 'An unexpected error occurred. Your journal entries are safe.'}
        </p>
        {error.digest && (
          <p className="text-xs text-[var(--color-text-muted)] font-mono">{error.digest}</p>
        )}
